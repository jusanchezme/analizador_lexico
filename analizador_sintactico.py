...
def ReglaA(tokens, anterior):
    # A -> ant A all
    if tokens[1].token_name ==  "ant":
        tokens.remove(1)
        ReglaA(tokens)
        
    if isValid(anterior) and tokens[1].token_name == "all"

def emparejar(tokens, terminal):
    if tokens[1].token_name == terminal:
        tokens.remove(1)
        return tokens
    Error error =  Error("Error lexico", token[1].row + 1, token[1].column + 1)
    

    ReglaSub -> Sub token.[1]Id  ->
        def ReglaSub(tokens):
            #[Sub, ID, left_parent, Id, Right_parent]
            if tokens[1].token_name == "Sub":
                tokens = emparejar(tokens, "Sub") #[ID, left_parent, Id, Right_parent]
                if tokens[1].token_name == "id":  
                    tokens = emparejar(tokens, "id") #[left_parent, Id, Right_parent]
